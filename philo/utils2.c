...
/*   Updated: 2026/05/25 12:02:41 by jpaulo-b         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_death(t_info *info)
{
	...
	while (++i < info->philo[0].num_of_philos)
	{
		pthread_mutex_lock(info->philo[i].meal_lock);
		now = get_time();
		if (info->philo[i].meals_eaten != info->philo[i].num_times_to_eat
			&& now - info->philo[i].last_meal > info->philo[i].time_to_die)
		{
